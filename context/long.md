...
## manifest-multi-copy
MULTIPLE WORKING COPIES OF ONE REPO (SHIPPED, commit 1ec3a96; refined for linked worktrees 2026-06-11). The projects manifest keys on (identity, path), not identity alone, so a repo checked out several times on one machine as SEPARATE CLONES keeps one manifest row PER COPY. Before: UNIQUE(identity) made a second copy overwrite the first row and silently drop it from save/restore. Linked git worktrees are intentionally NOT separate memory copies: project-local writable state resolves to the main worktree's root, so all linked worktrees share the main checkout's .engram/mem.db and tool candidates.

DESIGN CHOICE (rejected the alternative): we did NOT move project memory into the global DB keyed by identity. That would force INDEPENDENT CLONES to share short-term/events -- destroying the clone isolation that is useful for parallel long-running checkouts -- and is blind to per-copy local go.mod replace-directive wiring (repo1 -> ../repo0). Independent clones stay fully independent; linked worktrees share intentionally because they are one git repository with multiple checked-out branches.

MOVE vs COPY resolved deterministically at registration, NO --force flag, NO prompt (RegisterProject runs non-interactively from DB-open and hooks). RegisterProject first resolves ProjectStorageRoot: for linked git worktrees this is the main worktree root; for ordinary repos/clones it is the repo root. It also deletes stale live manifest rows for linked worktree paths that canonicalize to the same storage root, so old worktree-local .engram dirs do not make save snapshot the shared main DB twice. relocateMovedProject: when a new (identity, path) arrives and the identity has EXACTLY ONE existing row whose .engram is gone from disk -> it moved, relocate that row in place. A surviving old .engram (genuine 2nd clone) or multiple existing rows (ambiguous) -> plain insert. Stat error != NotExist is treated as present (relocate only on confirmed absence). Backstop: save-time dead-row prune (manifestEntries) reaps stale rows, now scoped to (identity, path).

RESTORE/REHYDRATE: each saved independent copy is staged separately; linked worktrees should have only the main checkout copy because DBPath/OpenProjectDB/RegisterProject collapse them to ProjectStorageRoot. apply/discard disambiguate with --slot <name> or --from <original-path>; ambiguous --apply lists candidates and stops. The per-copy summary is AGENT-composed (inject surfaces identity/slot/original-path/stage/last_seen as raw materials; the agent reads the staged mem.db and narrates) -- engram never hands back a canned summary string. Schema migration 1->2 drops UNIQUE(identity), adds UNIQUE(identity, path); safe because v1 guaranteed no duplicate identities. See [[dump-restore-all-design]], [[engram-architecture-invariants]], [[engram-design-portability]].
